<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        if (!empty($username) && !empty($password) && is_numeric($userID)) {
            ...
            $query = "SELECT UserId, Username, password FROM registration WHERE UserId = ? AND Username = ? LIMIT 1";
            ...
            if ($result && mysqli_num_rows($result) > 0) {
                ...
                echo "<script type='text/javascript'>alert('Invalid username or user ID')</script>";
            }
        } else {
